...
from src.models.LSTMSeq2Seq import LSTMSeq2Seq
...
@dataclass
class LSTMForecast(ProbForecastExp, LSTMParameters):
    model_type: str = "LSTM_forecast"
    ...
    def _process_train_batch(self, batch_x, batch_y, x_future):
        ...
        """
        B = batch_x.size(0)
        # Decoder input: future X + zeros for Y
        dec_inp = torch.cat([
            x_future,
            torch.zeros(B, self.pred_len, 1, device=self.device),
        ], dim=-1)  # (B, O, 43)

        pred = self.model(batch_x, dec_inp)  # (B, O, 1)
    ...
    def _process_val_batch(self, batch_x, batch_y, x_future, batch_x_date_enc, batch_y_date_enc):
        ...
        # MC Dropout: enable dropout, run multiple passes
        self.model.eval()
