...
class STGCNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_nodes, A_hat):
        ...
        self.odeg = ODEG(out_channels[-1], 365, A_hat, time=6)
...
class STGODE(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, adj_matrix, remap_matrix=None, recur_dropout=0, dropout=0, return_states=False, device='cuda',
        ...
        num_features = 7
        num_timesteps_input = 365
        num_timesteps_output = 365
        A_sp_hat = torch.tensor(adj_matrix, dtype=torch.float32, device=device)
        A_se_hat = None
    ...
    def forward(self, x):
        ...
        """
        x = x.unsqueeze(0)  # 添加 batch_size 维度，变成 (1, 455, 365, 7)
        ...
        x = self.pred(x)  # (1, 455, 365)
        x = x.unsqueeze(-1)  # (1, 455, 365, 1)
        x = x.squeeze(0)  # (455, 365, 1)
        return x
