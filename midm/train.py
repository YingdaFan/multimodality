"""
Training for MIDM with position-invariant denoiser (pure, no X).

No pseudo-masking. Like LSTM, ALL basins participate in training:
  - Observed basins: have Y, obs_flag=1, contribute to loss
  - Masked basins: Y=0, obs_flag=0, participate via spatial attention

Usage:
    python train.py --npz_path ../data_processing/data/prepped.npz \
...
def load_split(npz_path, split, y_basin_mean=None, y_basin_std=None):
    ...
    if y_basin_mean is None or y_basin_std is None:
        ...
        y_basin_std = np.maximum(y_basin_std, 1e-6)
    ...
    return {
        ...
        'y_basin_mean': y_basin_mean.astype(np.float32),
        'y_basin_std': y_basin_std.astype(np.float32),
    }
...
def train_epoch(model, schedule, data, masked_idx, non_masked_idx, device,
    ...
    """Hybrid: pseudo-masking + all observed basins contribute to loss."""
    model.train()
    losses = []
    y_all = data['y_norm']
    ...
    for win in np.random.permutation(data['n_windows']):
        for _ in range(n_repeats):
            ...
            noise_pred = model.predict_noise(y_noisy, y_prev_c, obs_mask, t)
...
def predict_all(model, schedule, data, masked_idx, non_masked_idx, device,
    ...
    model.eval()
    y_all, y_raw_all = data['y_norm'], data['y_raw']
    y_basin_mean = data['y_basin_mean']
    y_basin_std = data['y_basin_std']
    ...
    for win in range(n_win):
        ...
        pred_np = accum.squeeze(0).cpu().numpy()
...
def main():
    parser = argparse.ArgumentParser(description='MIDM pure (no X)')
    ...
    print(f'  {trn["n_windows"]}win x {trn["n_basins"]}bas x {trn["seq_len"]}steps')
