...
class ConditionalGuidedModel(nn.Module):
    def __init__(self, config, MTS_args):
        ...
        # Use c_out for diffusion target dimension (defaults to enc_in for multivariate)
        c_out = getattr(MTS_args, 'c_out', MTS_args.enc_in)
        data_dim = c_out * 2 if self.cat_y_pred else c_out

        self.lin1 = ConditionalLinear(data_dim, 128, n_steps)
        self.lin2 = ConditionalLinear(128, 128, n_steps)
        self.lin3 = ConditionalLinear(128, 128, n_steps)
        self.lin4 = nn.Linear(128, c_out)
