...
class NSDiffEarlyStopping(EarlyStopping):
    def save_checkpoint(self, val_loss, model):
        ...
        if 'prior_alpha_logit' in model:
            torch.save(model['prior_alpha_logit'], os.path.join(self.path, 'prior_alpha_logit.pth'))
...
class NsDiffCAMELSForecast(ProbForecastExp, NsDiffParameters):
    model_type: str = "NsDiff_CAMELS_forecast_twoprior"
    ...
    def _init_model(self):
        ...
        # Two-prior fusion: learnable α for blending f(x) and Y_imputed
        # α in logit space → sigmoid(α) ∈ (0, 1)
        # y_T_mean = sigmoid(α) * f(x) + (1 - sigmoid(α)) * Y_imputed_future
        self.has_imputed = getattr(self.dataset, 'has_imputed', False)
        if self.has_imputed:
            self.prior_alpha_logit = torch.nn.Parameter(
                torch.tensor(0.0, device=self.device)  # init α=0.5 (sigmoid(0)=0.5)
            )
            print(f"Two-prior fusion enabled: α initialized at sigmoid(0)=0.5")
    ...
    def _init_optimizer(self):
        # Models + prior_alpha jointly optimized
        param_groups = [
        ...
        if self.has_imputed:
            param_groups.append({'params': [self.prior_alpha_logit], 'lr': self.lr})
    ...
    def _save_run_check_point(self, seed):
        ...
        if self.has_imputed:
            self.run_state["prior_alpha_logit"] = self.prior_alpha_logit.data
    ...
    def _load_best_model(self):
        ...
        if self.has_imputed:
            alpha_path = os.path.join(self.run_save_dir, 'prior_alpha_logit.pth')
            if os.path.exists(alpha_path):
                self.prior_alpha_logit.data = torch.load(alpha_path, map_location=self.device)
    ...
    def _resume_run(self, seed):
        ...
        if self.has_imputed and "prior_alpha_logit" in check_point:
            self.prior_alpha_logit.data = check_point["prior_alpha_logit"]
    ...
    def _process_train_batch(self, batch_x, batch_y, x_future, batch_x_mark, batch_y_mark):
        """
        Forecasting training batch processing (Two-Prior variant)

        Data format:
        - batch_x:  (B, T, n_feat+2) = [X, Y_imputed_history, Y_history]
        - batch_y:  (B, O, 1) = Y target (future streamflow)
        - x_future: (B, O, n_feat+1) = [future forcing, Y_imputed_future]

        3 losses + dual-prior fusion:
        - loss1: f(x) prediction MSE
        - loss2: g(x) sigma estimation MSE
        - kl_loss: diffusion KL divergence
        - y_T_mean = α·f(x) + (1-α)·Y_imputed_future (learnable α)
        """
        ...
        # Decoder input: future X known (+Y_imputed_future if present), Y unknown (zeros)
        ...
        # Two-prior fusion: blend f(x) with Y_imputed_future
        if self.has_imputed:
            y_imputed_future = x_future[:, :, -1:]  # (B, O, 1) last col of x_future
            alpha = torch.sigmoid(self.prior_alpha_logit)
            y_T_mean = alpha * y_0_hat_batch + (1 - alpha) * y_imputed_future
        else:
            y_T_mean = y_0_hat_batch  # (B, O, 1) fallback to pure f(x)
    ...
    def _process_val_batch(self, batch_x, batch_y, x_future, batch_x_mark, batch_y_mark):
        ...
        for i in range(self.diffusion_config.testing.n_z_samples // minisample):
            ...
            # Two-prior fusion: blend f(x) with Y_imputed_future
            if self.has_imputed:
                y_imputed_future = x_future[:, :, -1:]  # (B, O, 1)
                alpha = torch.sigmoid(self.prior_alpha_logit)
                y_T_mean_batch = alpha * y_0_hat_batch + (1 - alpha) * y_imputed_future
                y_T_mean_tile = y_T_mean_batch.repeat(repeat_n, 1, 1, 1)
    ...
    def run(self, seed=42) -> Dict[str, float]:
        ...
        while self.current_epoch < self.epochs:
            ...
            model_dict = {'model': self.model, 'cond_pred_model': self.cond_pred_model,
                          'cond_pred_model_g': self.cond_pred_model_g}
            if self.has_imputed:
                model_dict['prior_alpha_logit'] = self.prior_alpha_logit
            self.early_stopper(val_loss, model=model_dict)
