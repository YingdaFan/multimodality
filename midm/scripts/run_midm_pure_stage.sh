#!/bin/bash
set -e
# Single-fold pure MIDM (no X conditioning): train + evaluate
#
# Usage: bash run_midm_pure_stage.sh <target_basins...>

if [ $# -lt 1 ]; then
    echo "Usage: bash run_midm_pure_stage.sh <target_basins...>"
    exit 1
fi
...
echo "======================================"
echo "MIDM (Pure, No X) Spatial Extrapolation"
echo "======================================"
...
# Step 1: Train MIDM
# --------------------------------------------------
echo "[Step 1/2] Training MIDM (pure) ..."
cd "$MIDM_DIR"
