"""
DiffCal Gx Pipeline with Decoder NO Causal Mask

Key difference from diffcal_gx.py:
- Uses mu_backbone_dec_nomask.py (Decoder without causal mask)
- Decoder self-attention is bidirectional (can attend to all positions)
- Still has cross-attention between Encoder and Decoder

Architecture comparison:
- Original: Encoder -> Decoder (causal mask) -> Output
- This:     Encoder -> Decoder (NO causal mask, bidirectional) -> Output

Rationale:
- Calibration task doesn't need autoregressive generation
- Every position in Decoder can attend to all other positions
- Cross-attention still allows Decoder to query Encoder output

Usage:
    python diffcal_gx_dec_nomask.py runs --seeds='[1]'
"""
...
import src.layer.mu_backbone_dec_nomask as ns_Transformer  # Changed: use Decoder without causal mask
import argparse
...
class DiffCalRaw(ProbForecastExp, DiffCalRawParameters):
    model_type: str = "diffusion_gx_dec_nomask"  # Changed: indicate Decoder no causal mask variant
    ...
    def _init_data_loader(self, shuffle=False, fast_test=True, fast_val=True):
        ...
        print(f"\nGx Pipeline Configuration (Decoder No Causal Mask):")
        print(f"  Backbone: mu_backbone_dec_nomask (bidirectional Decoder)")
    ...
    def _init_model(self):
        ...
        # Use Decoder without causal mask backbone
        self.cond_pred_model = ns_Transformer.Model(self.args).float().to(self.device)
        self.cond_pred_model_g = None

        print(f"  cond_pred_model: mu_backbone_dec_nomask.Model (bidirectional Decoder)")
    ...
    def _save(self):
        print("Generating and saving predictions (Decoder No Causal Mask backbone)...")
