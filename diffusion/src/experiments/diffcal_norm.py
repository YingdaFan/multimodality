"""
DiffCal NORM Pipeline (Stage 2 works in per-basin normalized space)

Key differences from diffcal_raw.py:
1. Uses CAMELSLoaderNorm which separates y_obs_* (input) and y_raw_* (label)
2. Input: y_obs_* = LSTM predictions (per-basin normalized)
3. Label: y_raw_* = True observations (per-basin normalized)
4. Loss masking: masked basins excluded from loss (avoid information leakage)
5. Saves predictions in NORMALIZED scale (postprocess denormalizes using y_mean_vae/y_std_vae)

Data flow:
- Input: y_obs_* (LSTM predictions filled by Stage 1)
- Label: y_raw_* (True observations)
- Learning: LSTM_predictions -> True_observations (correction)
- Output: Predictions in normalized scale

Usage:
    python diffcal_norm.py runs --seeds='[1]' --masked_basin_ids 01022500 02069700
"""
...
# Import CAMELS NORM dataset and loader
from src.datasets.camels_dataset_norm import CAMELSNorm, CAMELSNpzDatasetNorm
from src.dataloader.camels_loader_norm import CAMELSLoaderNorm
...
@dataclass
class DiffCalNorm(ProbForecastExp, DiffCalNormParameters):
    """
    DiffCal for CAMELS NORM Pipeline (per-basin normalized space)

    Key features:
    - Input: y_obs_* (LSTM predictions, per-basin normalized)
    - Label: y_raw_* (True observations, per-basin normalized)
    - Loss masking: masked basins excluded from loss
    - Predictions saved in normalized scale
    """
    model_type: str = "diffusion_norm"
    dataset_type: str = "CAMELS"

    def _init_dataset(self):
        """Initialize CAMELS NORM dataset"""
        self.dataset = CAMELSNorm(npz_path=self.npz_path)

    def _init_data_loader(self, shuffle=False, fast_test=True, fast_val=True):
        """
        Use CAMELSLoaderNorm with per-basin normalization
        """
        self._init_dataset()
        ...
        print(f"\nNORM Pipeline Configuration:")
        print(f"  Input: y_obs_* (LSTM predictions, per-basin normalized)")
        print(f"  Label: y_raw_* (True observations, per-basin normalized)")
        print(f"  Learning: LSTM_predictions -> True_observations")
        print(f"  Predictions: saved in normalized scale")

        self.dataloader = CAMELSLoaderNorm(
            ...
            mask_mode='original'  # Keep original LSTM predictions for input
        )
    ...
    def _process_train_batch(self, batch_x, batch_y, batch_x_mark, batch_y_mark, is_masked=None):
        ...
        Args:
            batch_x: (B, T, 43) - condition = [X(42), Y_obs(1)] (LSTM predictions)
            batch_y: (B, O, 1) - label = Y_raw (True observations)
    ...
    def run(self, seed=42) -> Dict[str, float]:
        ...
        # The actual predictions are saved in _save(), metrics computed in postprocess
    ...
    def _predict(self, loader, desc="Predicting"):
        """Generate predictions in NORMALIZED scale (postprocess will denormalize)"""
        self.model.eval()
        self.cond_pred_model.eval()
        ...
        with torch.no_grad():
            for batch_x, batch_y, origin_x, origin_y, batch_x_date_enc, batch_y_date_enc, is_masked in tqdm(loader, desc=desc):
                ...
                pred_mean = preds.mean(dim=-1)  # (B, T, 1) - normalized scale

                # Keep predictions in NORMALIZED scale
                # postprocess_perseg_aligntime.py with --use_vae_stats will denormalize
    ...
    def _save(self):
        """Save predictions in NORMALIZED scale (postprocess denormalizes using y_mean_vae/y_std_vae)"""
        print("Generating and saving predictions (NORM pipeline - normalized scale)...")
        print(f"  Note: postprocess_perseg_aligntime.py with --use_vae_stats will denormalize")
        ...
        for partition, loader in [('trn', train_loader_no_shuffle)]:  # skip tst prediction
            ...
            print(f"  Value range: [{preds.min():.4f}, {preds.max():.4f}] (normalized scale)")
