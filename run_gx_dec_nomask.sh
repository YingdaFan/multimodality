#!/bin/bash
# Two-Stage LSTM+Diffusion K-fold Cross-Validation - Decoder No Causal Mask Pipeline
#
# Uses Decoder without causal mask for calibration task.
#
# Usage:
#   bash run_gx_dec_nomask.sh diffcal              # 22 folds, start from fold 3
#   bash run_gx_dec_nomask.sh diffcal 22 3 22      # 22 folds, run folds 3-22
#   bash run_gx_dec_nomask.sh scatter diffcal 22 3 22  # With scatter fusion

# Check arguments
if [ $# -lt 1 ]; then
    echo "Usage: bash run_gx_dec_nomask.sh [FUSION_TYPE] <MODEL_NAME> [NUM_FOLDS] [START_FOLD] [END_FOLD]"
    echo ""
    echo "Two-Stage Decoder No Causal Mask Pipeline: LSTM (X->Y) + Diffusion (bidirectional Decoder)"
    echo ""
    echo "Key features:"
    echo "  - Stage 2 uses Decoder without causal mask"
    echo "  - Decoder self-attention is bidirectional"
    echo "  - Still has cross-attention between Encoder and Decoder"
    echo ""
    echo "Supported models: diffcal (uses _gx_dec_nomask version)"
    echo "Optional FUSION_TYPE: scatter, interference, scatterinterference"
    echo ""
    echo "Examples:"
    echo "  bash run_gx_dec_nomask.sh diffcal              # Default: 22 folds, no fusion"
    echo "  bash run_gx_dec_nomask.sh diffcal 22 3 22      # Folds 3-22, no fusion"
    echo "  bash run_gx_dec_nomask.sh scatter diffcal 22 3 22  # With scatter fusion"
    echo "  bash run_gx_dec_nomask.sh interference diffcal 22 3 22  # With interference fusion"
    exit 1
fi
...
# Check if Decoder No Causal Mask model script exists
PYTHON_SCRIPT="$DIFFUSION_DIR/src/experiments/${MODEL_NAME}_gx_dec_nomask.py"
if [ ! -f "$PYTHON_SCRIPT" ]; then
    echo "ERROR: Decoder No Causal Mask model script not found: $PYTHON_SCRIPT"
    echo "Please create ${MODEL_NAME}_gx_dec_nomask.py first."
    exit 1
fi
...
echo "=========================================="
echo "LSTM + ${MODEL_NAME}_gx_dec_nomask Two-Stage K-Fold CV"
echo "=========================================="
if [ -n "$FUSION_TYPE" ]; then
    echo "Diffusion Model: ${MODEL_NAME}_gx_dec_nomask (Bidirectional Decoder + ${FUSION_TYPE} fusion)"
else
    echo "Diffusion Model: ${MODEL_NAME}_gx_dec_nomask (Bidirectional Decoder)"
fi
echo "Total folds: $NUM_FOLDS"
echo "Running folds: $START_FOLD to $END_FOLD"
echo "Hyperparam folds (skipped): 1-$NUM_HYPERPARAM_FOLDS"
echo ""
echo "DECODER NO CAUSAL MASK PIPELINE CONFIGURATION:"
echo "  - Stage 2 uses Decoder without causal mask"
echo "  - Decoder self-attention is bidirectional"
if [ -n "$FUSION_TYPE" ]; then
...
echo "  Stage 2: ${MODEL_NAME}_gx_dec_nomask (X + Y_obs -> Y_raw with bidirectional Decoder)"
echo "=========================================="
...
for fold in $(seq $START_FOLD $END_FOLD); do
    echo ""
    echo "=========================================="
    echo "Running Fold $fold/$NUM_FOLDS (Decoder No Causal Mask Pipeline)"
    echo "=========================================="
    ...
    # ==========================================
    # Stage 2: Diffusion (Decoder No Causal Mask version)
    # ==========================================
    echo ""
    echo "------------------------------------------"
    if [ -n "$FUSION_TYPE" ]; then
        echo "Stage 2: ${MODEL_NAME}_gx_dec_nomask Diffusion (Bidirectional Decoder + ${FUSION_TYPE} fusion)"
    else
        echo "Stage 2: ${MODEL_NAME}_gx_dec_nomask Diffusion (Bidirectional Decoder)"
    fi
    echo "------------------------------------------"
    bash "$DIFFUSION_SCRIPTS_DIR/run_gx_dec_nomask_stage2.sh" "$MODEL_NAME" $FUSION_TYPE $FOLD_BASINS
...
echo ""
echo "=========================================="
echo "LSTM + ${MODEL_NAME}_gx_dec_nomask Folds $START_FOLD-$END_FOLD Complete!"
echo "=========================================="
echo "Decoder No Causal Mask Pipeline: Stage 2 used bidirectional Decoder"
...
if [ $END_FOLD -lt $NUM_FOLDS ]; then
    ...
    echo "  bash run_gx_dec_nomask.sh $MODEL_NAME $NUM_FOLDS $((END_FOLD+1)) $NUM_FOLDS"
fi
echo "=========================================="
