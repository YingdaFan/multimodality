"""
DiffCal Gx Pipeline with Balance Loss Weighting
...
4. Balance Loss Weighting: non-target basins with similar distribution
   to target basins get higher loss weight, helping model learn patterns
   more relevant to target (masked) basins

Loss weighting strategy:
- Compute target basins' distribution center: (mean_target, std_target)
- For each non-target basin, compute similarity to target distribution
- similarity = exp(-distance² / 2σ²) where distance = sqrt((mean_diff)² + (std_diff)²)
- Loss = sum(weight_i * loss_i) / sum(weight_i) for non-masked basins
...
class BalanceLossWeighter:
    ...
    Strategy:
    - Compute target basins' distribution center: (mean_target, std_target)
    - For each non-target basin, compute similarity: exp(-distance² / 2σ²)
    - Distance = sqrt((mean_diff)² + (std_diff)²)
    - Similar basins get higher loss weight, so model focuses on patterns relevant to target basins
    """
    def __init__(self, sigma=0.5, min_weight=0.1, max_weight=2.0):
        """
        Args:
            sigma: bandwidth parameter for Gaussian kernel (controls how fast similarity decays with distance)
            min_weight: minimum loss weight (for very dissimilar basins)
            max_weight: maximum loss weight (for very similar basins)
        """
        self.sigma = sigma
        self.min_weight = min_weight
        self.max_weight = max_weight

        # Will be set during training initialization
    ...
    def initialize_from_dataset(self, dataset, masked_basin_ids):
        """
        Compute target basins' distribution center from dataset.

        Args:
            dataset: CAMELSRaw dataset (must have basin_ids, y_mean_vae, y_std_vae)
            masked_basin_ids: list of target basin IDs (strings)
        """
        ...
        # Get basin IDs from dataset
        basin_ids = dataset.basin_ids  # list of basin ID strings
        ...
        # Get y_mean_vae and y_std_vae for target basins
        y_mean_vae_all = dataset.y_mean_vae  # (num_basins,)
        y_std_vae_all = dataset.y_std_vae    # (num_basins,)

        target_means = y_mean_vae_all[target_indices]
        target_stds = y_std_vae_all[target_indices]

        # Compute distribution center
        ...
        print(f"  Target means range: [{target_means.min():.4f}, {target_means.max():.4f}]")
        print(f"  Target stds range: [{target_stds.min():.4f}, {target_stds.max():.4f}]")

        self.initialized = True

    def compute_weights(self, batch_x, device):
        """
        Compute loss weights for a batch based on similarity to target distribution.

        Args:
            batch_x: (B, T, enc_in) where enc_in = [X(42), y_mean_vae(1), y_std_vae(1), y_obs(1)]
            device: torch device

        Returns:
            weights: (B,) tensor of loss weights
        """
        if not self.initialized:
            # Return uniform weights if not initialized
            return torch.ones(batch_x.size(0), device=device)

        # Extract y_mean_vae and y_std_vae from batch_x
        # batch_x structure: [X(42), y_mean_vae(1), y_std_vae(1), y_obs(1)]
        y_mean_vae = batch_x[:, 0, -3]  # (B,) - take first timestep, constant across time
        y_std_vae = batch_x[:, 0, -2]   # (B,)

        # Compute distance to target distribution center
        ...
        distance_sq = mean_diff ** 2 + std_diff ** 2  # (B,)

        # Gaussian kernel similarity
        similarity = torch.exp(-distance_sq / (2 * self.sigma ** 2))  # (B,)

        # Map similarity [0, 1] to weight [min_weight, max_weight]
...
class DiffCalRawParameters:
    ...
    # Balance Loss Weighting: weight non-target basins by similarity to target basins
    balance_sigma: float = 0.5  # Gaussian kernel bandwidth (smaller = more selective)
    balance_min_weight: float = 0.01  # Min weight for very dissimilar basins
    balance_max_weight: float = 2.0  # Max weight for very similar basins
...
class DiffCalRaw(ProbForecastExp, DiffCalRawParameters):
    ...
    def _init_data_loader(self, shuffle=False, fast_test=True, fast_val=True):
        ...
        print(f"\nGx Pipeline Configuration:")
        print(f"  Using y_raw_* (original scale)")
        print(f"  Global Y normalization: mean={self.y_global_mean:.4f}, std={self.y_global_std:.4f}")
        print(f"  Balance Loss Weighting: sigma={self.balance_sigma}, min={self.balance_min_weight}, max={self.balance_max_weight}")
        ...
        # Initialize balance loss weighter
    ...
    def _process_train_batch(self, batch_x, batch_y, batch_x_mark, batch_y_mark, is_masked=None):
        ...
        # Compute per-sample loss for loss1
        ...
        # Compute balance loss weights based on similarity to target basins
        balance_weights = self.balance_weighter.compute_weights(batch_x, self.device)  # (B,)

        # Apply loss masking and balance weighting
        if is_masked is not None and is_masked.any():
            ...
            # No masking, use all samples with balance weights
