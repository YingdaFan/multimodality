...
@dataclass
class PatchTSTForecast(ProbForecastExp, PatchTSTParameters):
    model_type: str = "PatchTST_forecast"
    ...
    def _process_train_batch(self, batch_x, batch_y, batch_x_date_enc, batch_y_date_enc):
        """
        PatchTST is encode-only: takes history, outputs all features.
        Extract last feature (streamflow) for loss.
        """
        # PatchTST forward: (x_enc, x_mark_enc, x_dec, x_mark_dec)
        # x_dec and x_mark_dec are not used by PatchTST but required by interface
        dummy_dec = torch.zeros(
            batch_x.size(0), self.pred_len, batch_x.size(2),
            device=self.device
        )
        pred = self.model(
            batch_x, batch_x_date_enc,
            dummy_dec, batch_y_date_enc,
        )  # (B, O, 43)
        pred_y = pred[:, :, -1:]  # (B, O, 1) streamflow
