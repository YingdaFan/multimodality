#!/bin/bash
# Two-Stage LSTM+FlowMatching K-fold Cross-Validation - Pure Encoder Pipeline (Hourly)
#
# Hourly CAMELS-H variant. Parallel to run_gx_enc_hourly.sh but Stage 2 uses
# flow matching (fmcal_gx_enc.py) instead of diffusion (diffcal_gx_enc.py).
#
# Outputs are written to diffusion/output_fm/ to avoid colliding with the
# diffusion pipeline's diffusion/output/. LSTM CSVs at lstm/output/ are still
# shared with the diffusion pipeline -- if you alternate between FM and
# diffusion runs on the same folds, expect those CSVs to be overwritten.
#
# Env vars (FM-specific):
#   FM_SOURCE_SIGMA  - source noise scale (default: -1 = auto-estimate)
#   FM_STEPS         - inference Euler steps (default: 20)
#
# Usage:
#   bash run_fm_gx_enc_hourly.sh fmcal              # 10 folds, start from fold 3
#   bash run_fm_gx_enc_hourly.sh fmcal 10 3 10      # 10 folds, run folds 3-10
#   bash run_fm_gx_enc_hourly.sh scatter fmcal 10 3 10  # With scatter fusion

if [ $# -lt 1 ]; then
    echo "Usage: bash run_fm_gx_enc_hourly.sh [FUSION_TYPE] <MODEL_NAME> [NUM_FOLDS] [START_FOLD] [END_FOLD]"
    echo ""
    echo "Two-Stage Pure Encoder FM Pipeline (Hourly): LSTM (X->Y) + FlowMatching (Pure Encoder)"
    echo ""
    ...
    echo ""
    echo "Key features:"
    echo "  - Stage 1 identical to run_gx_enc_hourly.sh (LSTM training + RAW NPZ fill)"
    echo "  - Stage 2 uses flow matching with informed source (LSTM prior as p_0)"
    echo "  - Outputs isolated to diffusion/output_fm/"
    echo ""
    echo "Supported models: fmcal (uses _gx_enc flow matching version)"
    echo "Optional FUSION_TYPE: scatter, interference, scatterinterference"
    echo ""
    echo "Examples:"
    echo "  bash run_fm_gx_enc_hourly.sh fmcal              # Default: 10 folds, no fusion"
    echo "  bash run_fm_gx_enc_hourly.sh fmcal 10 3 10      # Folds 3-10, no fusion"
    echo "  bash run_fm_gx_enc_hourly.sh scatter fmcal 10 3 10  # With scatter fusion"
    echo "  FM_SOURCE_SIGMA=0.2 bash run_fm_gx_enc_hourly.sh fmcal  # Fixed sigma_src"
    exit 1
fi
...
# FM output dir (isolated from diffusion's output/)
...
if [ ! -f "$PYTHON_SCRIPT" ]; then
    echo "ERROR: FM model script not found: $PYTHON_SCRIPT"
...
# Reset FM CSV files only when starting from fold 3.
# Note: LSTM CSVs are SHARED with the diffusion pipeline; we still reset them
# at fold 3 since each fold's Stage 1 retrains LSTM from scratch.
...
echo "=========================================="
echo "LSTM + ${MODEL_NAME}_gx_enc (FlowMatching) Two-Stage K-Fold CV (HOURLY)"
echo "=========================================="
...
if [ -n "$FUSION_TYPE" ]; then
    echo "FM Model: ${MODEL_NAME}_gx_enc (Pure Encoder + ${FUSION_TYPE} fusion)"
else
    echo "FM Model: ${MODEL_NAME}_gx_enc (Pure Encoder)"
fi
...
echo ""
echo "FLOW MATCHING PIPELINE CONFIGURATION:"
echo "  - Stage 2 uses Pure Encoder backbone (no Decoder)"
echo "  - Stage 2 trains via conditional flow matching with LSTM prior as p_0"
...
echo "  Stage 2: ${MODEL_NAME}_gx_enc Flow Matching (X + Y_obs -> Y_raw, Pure Encoder)"
echo "=========================================="
...
for fold in $(seq $START_FOLD $END_FOLD); do
    echo ""
    echo "=========================================="
    echo "Running Fold $fold/$NUM_FOLDS (Pure Encoder FM Pipeline - Hourly)"
    echo "=========================================="
    ...
    # ==========================================
    # Stage 2: Flow Matching (Pure Encoder)
    # ==========================================
    echo ""
    echo "------------------------------------------"
    if [ -n "$FUSION_TYPE" ]; then
        echo "Stage 2: ${MODEL_NAME}_gx_enc FlowMatching (Pure Encoder + ${FUSION_TYPE} fusion)"
    else
        echo "Stage 2: ${MODEL_NAME}_gx_enc FlowMatching (Pure Encoder)"
    fi
    echo "------------------------------------------"
    bash "$DIFFUSION_SCRIPTS_DIR/run_fm_gx_enc_stage2.sh" "$MODEL_NAME" $FUSION_TYPE $FOLD_BASINS
...
echo ""
echo "=========================================="
echo "LSTM + ${MODEL_NAME}_gx_enc (FlowMatching) Folds $START_FOLD-$END_FOLD Complete! (HOURLY)"
echo "=========================================="
echo "Pure Encoder FM Pipeline: Stage 2 used flow matching with LSTM prior as p_0"
...
if [ $END_FOLD -lt $NUM_FOLDS ]; then
    ...
    echo "  bash run_fm_gx_enc_hourly.sh $MODEL_NAME $NUM_FOLDS $((END_FOLD+1)) $NUM_FOLDS"
fi
echo "=========================================="
