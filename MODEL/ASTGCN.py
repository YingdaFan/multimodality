...
class ASTGCN(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, adj_matrix, remap_matrix=None, recur_dropout=0, dropout=0, return_states=False, device='cuda',
        ...
        nb_time_filter = 64
        time_strides = 1
        cheb_polynomials = torch.from_numpy(adj_matrix).float().to(device)
        num_for_predict = 365 
        len_input = 365 
        num_of_vertices = adj_matrix.shape[0]
    ...
    def forward(self, x):
        ...
        '''
        x = x.permute(0, 2, 1)  # 交换维度，变成 (455, 7, 365)
        x = x.unsqueeze(0)  # 添加 batch_size 维度，变成 (1, 455, 7, 365)
