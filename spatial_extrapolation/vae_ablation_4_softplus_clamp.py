...
# Section 0: Data Preprocessing Configuration
...
# Section 1: VAE Model Architecture
...
# Section 2: Conditional VAE Variant (Recommended)

class ConditionalBasinVAE(nn.Module):
    """
    Conditional VAE: more suitable for missing data scenarios
    Latent variable z captures shared patterns across basins, conditional variable c is basin features
    """
    ...
    def __init__(self,
        ...
        # No final ReLU in feature_encoder
    ...
    def decode(self, z: torch.Tensor, features: torch.Tensor) -> torch.Tensor:
        """Decode: generate flow statistics y based on latent code z and basin features x"""
        feature_h = self.feature_encoder(features)
        combined = torch.cat([z, feature_h], dim=1)
        output = self.decoder(combined)

        # Softplus constraint on std
        flow_mean = output[:, 0:1]
        flow_std_raw = output[:, 1:2]
        flow_std = F.softplus(flow_std_raw) + 1e-6

        flow_stats = torch.cat([flow_mean, flow_std], dim=1)
        return flow_stats
...
def vae_loss(outputs: Dict[str, torch.Tensor],
    ...
    Parameters:
    - beta: weight of KL term (beta-VAE)
    """
