#!/bin/bash
# Two-Stage LSTM+Diffusion K-fold Cross-Validation - Gx Pipeline
#
# Key differences from run_raw.sh:
# - Stage 2 uses Basin-Adaptive gx (learned from input statistics)
# - gx = BasinVarianceEstimator(X, Y_obs) instead of gx = 1
# - Can adapt diffusion noise scale per-basin
#
# Pipeline:
#   Stage 1: LSTM predicts normalized Y, denormalize, fill y_obs_*
#   Stage 2: Diffusion with learned gx works in original space
#
# Usage:
#   bash run_gx.sh diffcal              # 22 folds, start from fold 3
#   bash run_gx.sh diffcal 22 3 22      # 22 folds, run folds 3-22
#   bash run_gx.sh scatter diffcal 22 3 22  # With scatter fusion

# Check arguments
if [ $# -lt 1 ]; then
    echo "Usage: bash run_gx.sh [FUSION_TYPE] <MODEL_NAME> [NUM_FOLDS] [START_FOLD] [END_FOLD]"
    echo ""
    echo "Two-Stage Gx Pipeline: LSTM (X->Y) + Diffusion (X+Y_raw->Y_raw with learned gx)"
    echo ""
    echo "Key features:"
    echo "  - Stage 2 uses Basin-Adaptive gx"
    echo "  - gx = BasinVarianceEstimator(X, Y_obs)"
    echo "  - Adapts diffusion noise scale per-basin"
    echo ""
    echo "Supported models: diffcal (uses _gx version)"
    echo "Optional FUSION_TYPE: scatter, interference, scatterinterference"
    echo ""
    echo "Examples:"
    echo "  bash run_gx.sh diffcal              # Default: 22 folds, no fusion"
    echo "  bash run_gx.sh diffcal 22 3 22      # Folds 3-22, no fusion"
    echo "  bash run_gx.sh scatter diffcal 22 3 22  # With scatter fusion"
    echo "  bash run_gx.sh interference diffcal 22 3 22  # With interference fusion"
    exit 1
fi
...
# Check if Gx model script exists
PYTHON_SCRIPT="$DIFFUSION_DIR/src/experiments/${MODEL_NAME}_gx.py"
if [ ! -f "$PYTHON_SCRIPT" ]; then
    echo "ERROR: Gx model script not found: $PYTHON_SCRIPT"
...
echo "=========================================="
echo "LSTM + ${MODEL_NAME}_gx Two-Stage K-Fold CV"
echo "=========================================="
if [ -n "$FUSION_TYPE" ]; then
    echo "Diffusion Model: ${MODEL_NAME}_gx (Basin-Adaptive gx + ${FUSION_TYPE} fusion)"
else
    echo "Diffusion Model: ${MODEL_NAME}_gx (Basin-Adaptive gx)"
fi
...
echo "  - Stage 2 uses ORIGINAL Y scale"
echo "  - gx = BasinVarianceEstimator(X, Y_obs)"
echo "  - Adapts diffusion noise scale per-basin"
if [ -n "$FUSION_TYPE" ]; then
...
echo "  Stage 2: ${MODEL_NAME}_gx (X + Y_obs -> Y_raw with learned gx)"
echo "=========================================="
...
for fold in $(seq $START_FOLD $END_FOLD); do
    echo ""
    echo "=========================================="
    echo "Running Fold $fold/$NUM_FOLDS (Gx Pipeline)"
    echo "=========================================="
    ...
    # ==========================================
    # Stage 2: Diffusion (Gx version)
    # ==========================================
    echo ""
    echo "------------------------------------------"
    if [ -n "$FUSION_TYPE" ]; then
        echo "Stage 2: ${MODEL_NAME}_gx Diffusion (Basin-Adaptive gx + ${FUSION_TYPE} fusion)"
    else
        echo "Stage 2: ${MODEL_NAME}_gx Diffusion (Basin-Adaptive gx)"
    fi
    echo "------------------------------------------"
    bash "$DIFFUSION_SCRIPTS_DIR/run_gx_stage2.sh" "$MODEL_NAME" $FUSION_TYPE $FOLD_BASINS
...
echo ""
echo "=========================================="
echo "LSTM + ${MODEL_NAME}_gx Folds $START_FOLD-$END_FOLD Complete!"
echo "=========================================="
echo "Gx Pipeline: Stage 2 used Basin-Adaptive gx"
...
if [ $END_FOLD -lt $NUM_FOLDS ]; then
    ...
    echo "  bash run_gx.sh $MODEL_NAME $NUM_FOLDS $((END_FOLD+1)) $NUM_FOLDS"
fi
echo "=========================================="
