"""
Colorado Data Preprocessing Script - Modified version
With correct implementation of temporal and cumulative features
"""
...
def load_colorado_data(csv_path):
    ...
    df = pd.read_csv(csv_path, dtype={'basin_id': str})
...
def add_cumulative_features(ds, window_sizes=[7, 14, 30]):
    """Add cumulative precipitation and temperature features"""
    for window in window_sizes:
        # Cumulative precipitation
        ds[f'precip_sum_{window}d'] = ds['precipitation'].rolling(
            date=window, min_periods=1
        ).sum()
        
        # Mean temperature
        ds[f'temp_mean_{window}d'] = ds['temperature'].rolling(
            date=window, min_periods=1
        ).mean()
        
        # Cumulative snowmelt potential
        temp_positive = xr.where(ds['temperature'] > 0, ds['temperature'], 0)
...
def prep_data(
    ...
    """Preprocessing function - with all improvements"""
    ...
    # Original standardization method
