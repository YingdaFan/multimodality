...
# F is no longer needed since BasinVarianceEstimator no longer uses softplus
from torch.optim import *

# Inherit from Raw version
...
class BasinVarianceEstimator(nn.Module):
    """
    Per-basin gx estimator: directly uses the variance of y_obs as gx.

    Input: batch_x (B, T, enc_in)
    Output: gx_scale (B, 1, 1), broadcastable to (B, T, 1)

    gx = y_obs_std^2 (variance of the normalized LSTM predictions)

    This gives each basin its own gx, ensuring a reasonable signal-to-noise ratio:
    - High-flow basin: y_obs_std large -> gx large
    - Low-flow basin: y_obs_std small -> gx small
    ...
    def __init__(self, enc_in=45, hidden_dim=64, output_bias=1.0):
        super().__init__()
        self.enc_in = enc_in
        # hidden_dim and output_bias are kept for compatibility but no longer used

    def forward(self, batch_x):
        """
        Compute per-basin gx scale, directly based on the variance of y_obs.

        gx = y_obs_std^2 (clamped to a reasonable range)

        Returns:
            gx_scale: (B, 1, 1) - variance scaling factor
        """
        # Extract y_obs (normalized LSTM predictions)
        # batch_x structure: [X(42), y_mean_vae(1), y_std_vae(1), y_obs(1)]
        y_obs = batch_x[:, :, -1]  # (B, T) - last dimension is y_obs

        # Compute std along the time dimension for each sample
        y_obs_std = y_obs.std(dim=1) + 1e-6  # (B,)

        # gx = std^2
        gx_scale = y_obs_std ** 2  # (B,)

        # Clamp to reasonable range [0.1, 4.0]
        # 0.1: minimum variance, prevents noise from being too small
        # 4.0: maximum variance, prevents noise from overwhelming the signal
        gx_scale = gx_scale.clamp(min=0.1, max=4.0)

        return gx_scale.unsqueeze(-1).unsqueeze(-1)  # (B, 1, 1)
...
class DiffCalGx(DiffCalRaw, DiffCalRawParameters):
    ...
    - gx is no longer a constant 1, but is learned by BasinVarianceEstimator from (X, Y_obs)
    """
    ...
    def _init_model(self):
        ...
        # Added: store gx values for each epoch (for analysis)
        # gx_history list: each element is a (531,) array
        # Transposed to (531, epochs) when saving, i.e., each row is a basin, each column is an epoch
        self.gx_history = []

    def _init_optimizer(self):
        """Initialize optimizer (BasinVarianceEstimator no longer has trainable parameters)"""
        from torch_timeseries.utils.parse_type import parse_type
        from torch.amp import GradScaler

        # Note: basin_var_estimator now uses fixed y_obs_std^2 with no trainable parameters
    ...
    def _process_train_batch(self, batch_x, batch_y, batch_x_mark, batch_y_mark, is_masked=None):
        ...
        The only difference from the parent class: gx is computed by BasinVarianceEstimator.
        """
        from src.layer.nsdiff_utils import q_sample, cal_sigma_tilde, cal_forward_noise

        batch_y_target = batch_y

        # ===== Key change: compute gx using BasinVarianceEstimator =====
    ...
    def _compute_all_basin_gx(self):
        ...
        with torch.no_grad():
            # Iterate over the first batch of train_loader (contains all 531 basins)
    ...
    def _save_run_check_point(self, seed):
        ...
        # Compute and save gx values for the current epoch
        ...
        # Also save gx_history as numpy file (for analysis)
