...
from src.datasets.camels_dataset_forecast import CAMELSForecast
from src.dataloader.camels_loader_forecast import CAMELSForecastLoader
...
class NsDiffCAMELSForecast(ProbForecastExp, NsDiffParameters):
    ...
    def _init_dataset(self):
        ...
        self.dataset = CAMELSForecast(npz_path=self.npz_path)
    ...
    def _init_data_loader(self, shuffle=True, fast_test=True, fast_val=True):
        ...
        self.dataloader = CAMELSForecastLoader(
    ...
    def _init_model(self):
        ...
        args_dict = {
            ...
            "enc_in": self.dataset.num_features,  # 43: X(42) + Y(1)
    ...
    def _train(self):
        ...
        with torch.enable_grad(), tqdm(total=len(self.train_loader.dataset)) as progress_bar:
            ...
            for i, (
                ...
                try:
                    # Pure float32 — no autocast. float16 causes overflow in both
                    # Transformer attention and NsDiff diffusion model.
                    loss = self._process_train_batch(
                        batch_x, batch_y, batch_x_date_enc, batch_y_date_enc
                    )
                except RuntimeError as e:
                    if "NaN/Inf DETECTED" in str(e) or "mu_backbone NaN" in str(e):
                        print(f"\n[STOP] epoch={self.current_epoch}, step={i}, "
                              f"batch_x range=[{batch_x.min().item():.4g}, {batch_x.max().item():.4g}], "
                              f"batch_y range=[{batch_y.min().item():.4g}, {batch_y.max().item():.4g}]")
                        print(str(e))
                        raise  # stop training entirely
                    raise
    ...
    def _process_train_batch(self, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        Data format:
        - batch_x: (B, T, 43) = [X(42 dims), Y_history(1 dim)]
    ...
    def _process_val_batch(self, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        for i in range(self.diffusion_config.testing.n_z_samples // minisample):
            ...
            gen_y_box = []
            # Fixed: removed duplicate inner loop (upstream NsDiff bug)
            for _ in range(self.diffusion_config.testing.n_z_samples_depart):
                ...
                # Extract final diffusion step result directly (no intermediate storage)
    ...
    def _test_and_save(self):
        ...
        with torch.no_grad(), tqdm(total=len(self.test_loader.dataset)) as progress_bar:
            for batch_x, batch_y, origin_x, origin_y, batch_x_date_enc, batch_y_date_enc, *rest in self.test_loader:
                ...
                # Probabilistic metrics (normalized scale; postprocess handles denorm)
                metrics_preds = preds
                metrics_truths = truths
                metric_results.append(self.task_pool.apply_async(
                    update_metrics,
                    (metrics_preds.contiguous().cpu().detach(),
                     metrics_truths.contiguous().cpu().detach(),
                     self.metrics)
                ))

                # Save predictions (mean across samples)
