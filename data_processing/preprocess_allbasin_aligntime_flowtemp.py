"""
Flow Temperature Data Preprocessing Script - All-Basin Normalization with Time Alignment
Uses time-aligned batch organization AND all-basin (global) normalization for Y
Based on the DRB_NHD_on_NHM dataset for river temperature prediction preprocessing
"""
...
def add_temporal_features(ds):
    ...
    # Add sine/cosine encoding of month
...
def create_distance_matrix_from_basin_order(n_basins):
    """Create simple distance matrix based on basin order.
    Without lat/lon info, use basin index as distance proxy."""
...
def prep_data(
    ...
    """Preprocess Flow Temperature data - using global normalization and time-aligned batches

    Args:
        csv_file: Path to flowtemperature.csv
        ...
        add_temporal: Whether to add temporal features (month_sin, month_cos, etc.)
        add_cumulative: Whether to add cumulative features (rolling windows)
...
if __name__ == "__main__":
    ...
    # Usage: python preprocess_allbasin_aligntime_flowtemp.py [basin1] [basin2] ...
