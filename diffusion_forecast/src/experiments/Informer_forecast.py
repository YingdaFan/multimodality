...
@dataclass
class InformerForecast(ProbForecastExp, InformerParameters):
    model_type: str = "Informer_forecast"
    ...
    def _build_dec_input(self, batch_x, x_future):
        """
        Build decoder input with label (from history tail) + future features.
        Informer decoder: (B, label_len + pred_len, N)
        Future: known X + zeros for Y.
        """
        B = batch_x.size(0)
        N = batch_x.size(2)
        label = batch_x[:, -self.label_len:, :]  # (B, label_len, 43)
        future_full = torch.cat([
            x_future,
            torch.zeros(B, self.pred_len, 1, device=batch_x.device),
        ], dim=-1)  # (B, O, 43)
        return torch.cat([label, future_full], dim=1)  # (B, label_len+O, 43)

    def _build_dec_mark(self, batch_x_date_enc, batch_y_date_enc):
        """Build decoder date encoding: label portion + future portion."""
        label_mark = batch_x_date_enc[:, -self.label_len:, :]
        return torch.cat([label_mark, batch_y_date_enc], dim=1)
    ...
    def _process_train_batch(self, batch_x, batch_y, x_future, batch_x_date_enc, batch_y_date_enc):
        """
        Informer encoder-decoder with future X in decoder input.
        Training uses actual future Y in decoder (teacher forcing not needed since
        Informer uses generative-style decoding, but we feed zeros for Y).
        """
        dec_inp = self._build_dec_input(batch_x, x_future)
        dec_mark = self._build_dec_mark(batch_x_date_enc, batch_y_date_enc)
    ...
    def _process_val_batch(self, batch_x, batch_y, x_future, batch_x_date_enc, batch_y_date_enc):
        """
        MC Dropout inference with future X conditioning.
