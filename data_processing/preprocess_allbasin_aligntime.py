"""
Colorado Data Preprocessing Script - All-Basin Normalization with Time Alignment
...
def prep_data(
    ...
    """
    Preprocessing function with time-aligned batches AND all-basin (global) normalization
    ...
    # Dynamic features (time-varying meteorological drivers)
    x_vars = [
        ...
        'daylight_duration_s',          # daylight duration
        'snow_water_equivalent_kg_m2',  # snow water equivalent
        # Static features (constant per basin, time-invariant)
        'latitude',                     # latitude
        'longitude',                    # longitude
        'elevation',                    # elevation
    ]
    y_var = 'inflow'  # Target variable (target: reservoir inflow)
...
if __name__ == "__main__":
    ...
    val_dates = ('2011-01-01', '2011-12-31')
    test_dates = ('1999-01-01', '2009-12-31')

    # Parse command line arguments for basins to exclude
    # Usage: python preprocess_allbasin_aligntime.py [basin1] [basin2] ...
    ...
    if len(sys.argv) > 1:
        ...
        # Example: exclude_basins_list = ['BSR', 'CAU', 'CRY']
        exclude_basins_list = None  # Set to None to use all basins, or provide a list of basin names
    ...
    prep_data(
        csv_file='../../All_Reservoirs_Combined.csv',
        train_dates=train_dates,
        val_dates=val_dates,
        test_dates=test_dates,
        seq_len=365,
        offset=1.0,
        out_file=os.path.join(data_dir, 'prepped_colorado_allbasin_aligntime.npz'),
