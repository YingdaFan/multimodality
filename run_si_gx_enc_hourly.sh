#!/bin/bash
# Two-Stage LSTM+StochasticInterpolant K-fold Cross-Validation - Pure Encoder Pipeline (Hourly)
#
# Hourly CAMELS-H variant. Parallel to run_fm_gx_enc_hourly.sh but Stage 2
# uses the Stochastic Interpolant framework (sical_gx_enc.py), which provides
# a unified ODE/SDE family. fmcal is recovered as the SI_SIGMA_INT=0,
...
#
# Usage:
#   bash run_si_gx_enc_hourly.sh sical              # 10 folds, start from fold 3
#   bash run_si_gx_enc_hourly.sh sical 10 3 10      # folds 3-10
#   bash run_si_gx_enc_hourly.sh scatter sical 10 3 10  # with scatter fusion

if [ $# -lt 1 ]; then
    echo "Usage: bash run_si_gx_enc_hourly.sh [FUSION_TYPE] <MODEL_NAME> [NUM_FOLDS] [START_FOLD] [END_FOLD]"
    echo ""
    echo "Two-Stage Pure Encoder SI Pipeline (Hourly): LSTM (X->Y) + StochasticInterpolant"
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  bash run_si_gx_enc_hourly.sh sical                                          # default 10 folds"
    echo "  bash run_si_gx_enc_hourly.sh sical 10 3 10                                  # folds 3-10"
    echo "  SI_SIGMA_INT=0 SI_EPS_INFERENCE=0 bash run_si_gx_enc_hourly.sh sical 10 3 3 # sanity check (fmcal-equivalent)"
    echo "  SI_SIGMA_INT=0.3 SI_EPS_INFERENCE=0 bash run_si_gx_enc_hourly.sh sical      # default SI ODE branch"
    echo "  SI_SIGMA_INT=0.3 SI_EPS_INFERENCE=0.5 bash run_si_gx_enc_hourly.sh sical    # SI SDE branch"
    exit 1
fi
...
echo "=========================================="
echo "LSTM + ${MODEL_NAME}_gx_enc (StochasticInterpolant) Two-Stage K-Fold CV (HOURLY)"
echo "=========================================="
...
for fold in $(seq $START_FOLD $END_FOLD); do
    echo ""
    echo "=========================================="
    echo "Running Fold $fold/$NUM_FOLDS (Pure Encoder SI Pipeline - Hourly)"
    echo "=========================================="
...
echo ""
echo "=========================================="
echo "LSTM + ${MODEL_NAME}_gx_enc (StochasticInterpolant) Folds $START_FOLD-$END_FOLD Complete! (HOURLY)"
echo "=========================================="
...
if [ $END_FOLD -lt $NUM_FOLDS ]; then
    ...
    echo "  bash run_si_gx_enc_hourly.sh $MODEL_NAME $NUM_FOLDS $((END_FOLD+1)) $NUM_FOLDS"
fi
echo "=========================================="
