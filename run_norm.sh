#!/bin/bash
# Two-Stage LSTM+Diffusion K-fold Cross-Validation - NORM Pipeline
#
# Key differences from run_raw.sh:
# - Stage 2 works in PER-BASIN NORMALIZED space
# - Uses y_obs_* (LSTM normalized predictions) as input
# - Uses y_raw_* (true observations normalized) as label
# - Loss masking: masked basins excluded from loss
# - Postprocess uses --use_vae_stats to denormalize
#
# Pipeline:
#   Stage 1: LSTM predicts normalized Y, fill y_obs_* with normalized predictions
#   Stage 2: Diffusion learns LSTM_pred -> True_obs correction
#
# Usage:
#   bash run_norm.sh NsDiff              # 22 folds, start from fold 3
#   bash run_norm.sh NsDiff 22 3 22      # 22 folds, run folds 3-22

# Check arguments
if [ $# -lt 1 ]; then
    echo "Usage: bash run_norm.sh <MODEL_NAME> [NUM_FOLDS] [START_FOLD] [END_FOLD]"
    echo ""
    echo "Two-Stage NORM Pipeline: LSTM (X->Y) + Diffusion (X+Y_obs->Y_raw)"
    echo ""
    echo "Key features:"
    echo "  - Stage 2 works in PER-BASIN NORMALIZED space"
    echo "  - Input: y_obs_* (LSTM predictions)"
    echo "  - Label: y_raw_* (True observations)"
    echo "  - Loss masking: masked basins excluded from loss"
    echo "  - Postprocess uses --use_vae_stats to denormalize"
    echo ""
    echo "Supported models: NsDiff (uses _norm version)"
    echo ""
    echo "Examples:"
    echo "  bash run_norm.sh NsDiff              # Default: 22 folds"
    echo "  bash run_norm.sh NsDiff 22 3 22      # Folds 3-22"
    echo "  bash run_norm.sh NsDiff 22 5 10      # Folds 5-10 only"
    exit 1
fi
...
# Check if NORM model script exists
PYTHON_SCRIPT="$DIFFUSION_DIR/src/experiments/${MODEL_NAME}_norm.py"
if [ ! -f "$PYTHON_SCRIPT" ]; then
    echo "ERROR: NORM model script not found: $PYTHON_SCRIPT"
...
echo "=========================================="
echo "LSTM + ${MODEL_NAME}_norm Two-Stage K-Fold CV"
echo "=========================================="
echo "Diffusion Model: ${MODEL_NAME}_norm"
...
echo "  - Stage 2 uses PER-BASIN NORMALIZED space"
echo "  - Input: y_obs_* (LSTM predictions)"
echo "  - Label: y_raw_* (True observations)"
echo "  - Loss masking: masked basins excluded"
echo "  - Postprocess: --use_vae_stats for denormalization"
echo ""
echo "Pipeline per fold:"
echo "  Stage 1: LSTM (X -> Y) + fill y_obs_* (normalized)"
echo "  Stage 2: ${MODEL_NAME}_norm (X + Y_obs -> Y_raw)"
echo "=========================================="
...
for fold in $(seq $START_FOLD $END_FOLD); do
    echo ""
    echo "=========================================="
    echo "Running Fold $fold/$NUM_FOLDS (NORM Pipeline)"
    echo "=========================================="
    ...
    # ==========================================
    # Stage 1: LSTM (NORM version - fills y_obs_* with normalized predictions)
    # ==========================================
    echo "------------------------------------------"
    echo "Stage 1: LSTM Training & NORM NPZ Fill"
    echo "------------------------------------------"
    bash "$LSTM_DIR/run_camels_perstd_stage1.sh" $FOLD_BASINS

    # ==========================================
    # Stage 2: Diffusion (NORM version)
    # ==========================================
    echo ""
    echo "------------------------------------------"
    echo "Stage 2: ${MODEL_NAME}_norm Diffusion (per-basin normalized)"
    echo "------------------------------------------"
    bash "$DIFFUSION_SCRIPTS_DIR/run_norm_stage2.sh" "$MODEL_NAME" $FOLD_BASINS
...
echo ""
echo "=========================================="
echo "LSTM + ${MODEL_NAME}_norm Folds $START_FOLD-$END_FOLD Complete!"
echo "=========================================="
echo "NORM Pipeline: Stage 2 worked in per-basin normalized space"
...
if [ $END_FOLD -lt $NUM_FOLDS ]; then
    ...
    echo "  bash run_norm.sh $MODEL_NAME $NUM_FOLDS $((END_FOLD+1)) $NUM_FOLDS"
fi
echo "=========================================="
