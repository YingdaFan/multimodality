...
class NsDiffCAMELS(ProbForecastExp, NsDiffParameters):
    ...
    def _init_model(self):
        ...
        args_dict = {
            ...
            "enc_in": self.dataset.num_features,  # 43 for CAMELS
    ...
    def _process_train_batch(self, batch_x, batch_y, batch_x_mark, batch_y_mark):
        """
        Training batch processing - simplified version (sigma adjustment removed, for calibration tasks)
        """
        # === Original non-stationary variance adjustment (commented out, not needed for calibration tasks) ===
        # y_sigma = wv_sigma_trailing(torch.concat([batch_x, batch_y], dim=1), self.rolling_length)
        # y_sigma = y_sigma[:, -self.pred_len:, :] + EPS

        # === Simplified version: use fixed sigma = 1 ===
        gx = torch.ones_like(batch_y).to(self.device) + EPS
        y_sigma = torch.ones_like(batch_y).to(self.device) + EPS
        ...
        # === Original gx computation (commented out) ===
        # gx = self.cond_pred_model_g(batch_x) + EPS # (B, O, N)

        loss1 = (y_0_hat_batch - batch_y).square().mean()
        # === Original loss2 (commented out, sigma adjustment loss) ===
        ...
        # === Original loss (commented out) ===
        # loss = kl_loss + loss1 + loss2
        loss = kl_loss + loss1  # Simplified version: loss2 removed
        return loss

    def _process_val_batch(self, batch_x, batch_y, batch_x_mark, batch_y_mark):
        """
        Validation batch processing - simplified version (sigma adjustment removed, for calibration tasks)
        """
        b = batch_x.shape[0]
        ...
        # === Simplified version: use fixed sigma = 1 ===
        gx = torch.ones(batch_x.size(0), self.pred_len, self.dataset.num_features).to(self.device) + EPS
        ...
        # self.pred_len, self.dataset.num_features, self.diffusion_config.testing.n_z_samples)
        # return outs, batch_y
        expected_features = 1 if self.args.features == 'MS' else self.dataset.num_features
        assert (outs.shape[1], outs.shape[2], outs.shape[3]) == (
            self.pred_len, expected_features, self.diffusion_config.testing.n_z_samples)
        return outs, batch_y
