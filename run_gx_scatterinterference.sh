...
# Check arguments
if [ $# -lt 1 ]; then
    echo "Usage: bash run_gx.sh <MODEL_NAME> [NUM_FOLDS] [START_FOLD] [END_FOLD]"
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  bash run_gx.sh diffcal              # Default: 22 folds"
    echo "  bash run_gx.sh diffcal 22 3 22      # Folds 3-22"
    echo "  bash run_gx.sh diffcal 22 5 10      # Folds 5-10 only"
    exit 1
fi
...
for fold in $(seq $START_FOLD $END_FOLD); do
    ...
    echo "------------------------------------------"
    bash "$DIFFUSION_SCRIPTS_DIR/run_gx_stage2.sh" "$MODEL_NAME" scatterinterference $FOLD_BASINS
