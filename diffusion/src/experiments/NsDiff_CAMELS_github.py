...
import torch
from torch.cuda.amp import autocast, GradScaler  # FP16 mixed precision training
from tqdm import tqdm
...
class NsDiffCAMELS(ProbForecastExp, NsDiffParameters):
    ...
    def _init_optimizer(self):
        ...
        # FP16 mixed precision training - initialize GradScaler
    ...
    def _process_train_batch(self, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        # Decoder input: conditional features (43 dims)
        ...
        # NsDiff model: condition (43 dims) + noisy Y (1-dim) -> noise prediction (1-dim)
    ...
    def _save_predictions(self):
        ...
        Save format compatible with postprocess_perseg_aligntime.py
        """
        ...
        with torch.no_grad():
            for batch_x, batch_y, origin_x, origin_y, batch_x_date_enc, batch_y_date_enc in tqdm(self.test_loader, desc="Predicting"):
                batch_x = batch_x.to(self.device).float()
                batch_y = batch_y.to(self.device).float()
                batch_x_date_enc = batch_x_date_enc.to(self.device).float()
                batch_y_date_enc = batch_y_date_enc.to(self.device).float()
                ...
                # preds shape: (B, O, N, S) - B=batch, O=pred_len, N=features, S=samples
                # Take the mean of predictions as point estimate
                pred_mean = preds.mean(dim=-1)  # (B, O, N)

                all_preds.append(pred_mean.cpu().numpy())
                all_truths.append(truths.cpu().numpy())

        # Merge all batches
        all_preds = np.concatenate(all_preds, axis=0)  # (total_samples, pred_len, features)
        all_truths = np.concatenate(all_truths, axis=0)

        # Create fixed output directory (compatible with postprocess_perseg_aligntime.py format)
        # Use the output/pred folder under project root
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        output_dir = os.path.join(project_root, 'output')
        pred_dir = os.path.join(output_dir, 'pred')
        os.makedirs(pred_dir, exist_ok=True)

        # Save as tst.npy (format expected by postprocess script)
        pred_path = os.path.join(pred_dir, 'tst.npy')
        truth_path = os.path.join(output_dir, 'truths.npy')

        np.save(pred_path, all_preds)
        np.save(truth_path, all_truths)

        print(f"Predictions saved to {pred_path}")
        print(f"  Shape: {all_preds.shape}")
        print(f"Truths saved to {truth_path}")
        print(f"  Shape: {all_truths.shape}")
        print(f"\nYou can use the following command for postprocessing:")
        print(f"  python ../data_processing/postprocess_perseg_aligntime.py \\")
        print(f"    --pred_dir {pred_dir} --partition tst")
