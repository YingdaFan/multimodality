#!/bin/bash
# K-fold cross-validation for Neural Process baselines (ANP / GNP)
#
# Mirrors the structure of run_gx_enc.sh but replaces
# LSTM + Diffusion with a single NP model per fold.
#
# Usage:
#   bash run_np_kfold.sh anp              # 22 folds, start from fold 3
#   bash run_np_kfold.sh gnp 22 3 22      # 22 folds, folds 3-22
#   bash run_np_kfold.sh anp 22 3 5       # folds 3-5 only

if [ $# -lt 1 ]; then
    echo "Usage: bash run_np_kfold.sh <MODEL_TYPE> [NUM_FOLDS] [START_FOLD] [END_FOLD]"
...
for fold in $(seq $START_FOLD $END_FOLD); do
    ...
    # --- Preprocess (creates prepped.npz if needed) ---
    # The NPZ must already exist. If not, run the preprocessing step first.
    ...
    # --- Train & Evaluate ---
    bash "$NP_SCRIPTS/run_np_stage.sh" "$MODEL_TYPE" $FOLD_BASINS
...
echo "=========================================="
echo "NP K-Fold CV ($MODEL_TYPE) Complete!"
echo "=========================================="
