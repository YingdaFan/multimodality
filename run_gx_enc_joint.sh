#!/bin/bash
# Joint LSTM+Diffusion End-to-End K-fold Cross-Validation - Pure Encoder Pipeline
#
# Same as run_gx_enc.sh but Stage 2 uses joint training:
...
#
# Usage:
#   bash run_gx_enc_joint.sh diffcal              # 22 folds, start from fold 3
#   bash run_gx_enc_joint.sh diffcal 22 3 22      # 22 folds, run folds 3-22
#   bash run_gx_enc_joint.sh scatter diffcal 22 3 22  # With scatter fusion

# Check arguments
if [ $# -lt 1 ]; then
    echo "Usage: bash run_gx_enc_joint.sh [FUSION_TYPE] <MODEL_NAME> [NUM_FOLDS] [START_FOLD] [END_FOLD]"
    echo ""
    echo "Joint End-to-End Pipeline: LSTM (X->Y) + Diffusion (Joint, Pure Encoder)"
    echo ""
    echo "Key features:"
    echo "  - Stage 1: LSTM pre-training (same as run_gx_enc.sh)"
    ...
    echo ""
    echo "Examples:"
    echo "  bash run_gx_enc_joint.sh diffcal              # Default: 22 folds, no fusion"
    echo "  bash run_gx_enc_joint.sh diffcal 22 3 22      # Folds 3-22, no fusion"
    echo "  bash run_gx_enc_joint.sh scatter diffcal 22 3 22  # With scatter fusion"
    echo "  LSTM_LR=5e-6 bash run_gx_enc_joint.sh diffcal # Custom LSTM LR"
    exit 1
fi
...
echo "=========================================="
echo "Joint LSTM+Diffusion K-Fold CV (Pure Encoder)"
echo "=========================================="
...
echo "  - Stage 1: LSTM pre-training (same as before)"
...
for fold in $(seq $START_FOLD $END_FOLD); do
    echo ""
    echo "=========================================="
    echo "Running Fold $fold/$NUM_FOLDS (Joint Pipeline)"
    echo "=========================================="
...
echo ""
echo "=========================================="
echo "Joint LSTM+Diffusion Folds $START_FOLD-$END_FOLD Complete!"
echo "=========================================="
...
if [ $END_FOLD -lt $NUM_FOLDS ]; then
    ...
    echo "  bash run_gx_enc_joint.sh $MODEL_NAME $NUM_FOLDS $((END_FOLD+1)) $NUM_FOLDS"
fi
echo "=========================================="
