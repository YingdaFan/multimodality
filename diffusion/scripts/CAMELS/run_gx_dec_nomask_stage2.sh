#!/bin/bash
# Stage 2 of LSTM+Diffusion pipeline using Decoder without causal mask
#
# Usage: bash run_gx_dec_nomask_stage2.sh <MODEL_NAME> [FUSION_TYPE] <target_basins...>
#   FUSION_TYPE: interference, scatter, scatterinterference (optional)
#
# Example:
#   bash run_gx_dec_nomask_stage2.sh diffcal 01022500 02069700              # No wave fusion
#   bash run_gx_dec_nomask_stage2.sh diffcal interference 01022500 02069700 # With interference

# Check arguments
if [ $# -lt 2 ]; then
    echo "Usage: bash run_gx_dec_nomask_stage2.sh <MODEL_NAME> [FUSION_TYPE] <target_basins...>"
    echo ""
    ...
    echo ""
    echo "This script is Stage 2 (Decoder No Causal Mask) of the LSTM+Diffusion pipeline."
    echo ""
    echo "Examples:"
    echo "  bash run_gx_dec_nomask_stage2.sh diffcal 01022500 02069700              # No fusion"
    echo "  bash run_gx_dec_nomask_stage2.sh diffcal interference 01022500 02069700 # Interference"
    exit 1
fi
...
echo "======================================"
echo "Stage 2: Diffusion with Decoder No Causal Mask"
echo "======================================"
if [ -n "$FUSION_TYPE" ]; then
    echo "Model: diffusion_gx_dec_nomask with ${FUSION_TYPE} fusion"
    echo "Wave Fusion: ENABLED (bidirectional)"
else
    echo "Model: diffusion_gx_dec_nomask"
...
# Step 2: Train diffusion model (Decoder No Causal Mask version)
echo ""
echo "[Step 2/3] Training ${MODEL_NAME}_gx_dec_nomask model..."
echo "======================================"

# Build Python script path (use gx_dec_nomask version)
PYTHON_SCRIPT="./src/experiments/${MODEL_NAME}_gx_dec_nomask.py"
...
# Train with Decoder No Causal Mask backbone
...
python postprocess_perseg_aligntime_raw.py \
    ...
    --model_name "diffusion_gx_dec_nomask" \
    --partition trn \
...
echo ""
echo "======================================"
echo "Stage 2 (Decoder No Causal Mask) Complete! (Model: diffusion_gx_dec_nomask)"
echo "======================================"
echo "Target basins: ${TARGET_BASINS[*]}"
