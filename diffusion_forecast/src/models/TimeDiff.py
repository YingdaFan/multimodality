...
class TimeDiff(nn.Module):
    ...
    def _build_future_cond(self, linear_outputs_i, x_dec_i):
        """Build future conditioning: actual X for known covariates, linear trend for target Y."""
        future_cond = linear_outputs_i.permute(0, 2, 1).clone()  # (B, N, O)
        n_x = getattr(self.args, 'n_x_features', None)
        if n_x is not None and n_x > 0:
            # Replace first n_x features (known covariates) with actual future values
            future_cond[:, :n_x, :] = x_dec_i[:, -self.pred_len:, :n_x].permute(0, 2, 1)
        return future_cond
    ...
    def train_forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        future_cond = self._build_future_cond(linear_outputs_i, x_dec_i)
        x_past = torch.cat([x_past, future_cond], dim=-1)
