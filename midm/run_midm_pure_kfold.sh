#!/bin/bash
# K-fold cross-validation for pure MIDM (no X conditioning)
#
# Usage:
#   bash run_midm_pure_kfold.sh              # 22 folds, start from fold 3
#   bash run_midm_pure_kfold.sh 22 3 22      # 22 folds, folds 3-22
#   bash run_midm_pure_kfold.sh 22 3 5       # folds 3-5 only
...
for fold in $(seq $START_FOLD $END_FOLD); do
    ...
    bash "$MIDM_SCRIPTS/run_midm_pure_stage.sh" $FOLD_BASINS
...
echo "=========================================="
echo "MIDM (Pure) K-Fold CV Complete!"
echo "=========================================="
